...
const PainelLimpezaScreen: React.FC<PainelLimpezaScreenProps> = ({ onReturnToDashboard, onGoToAdmin }) => {
  ...
  return (
    <div className="min-h-screen bg-[#EDF1F6] text-[#0F2A52] flex flex-col font-sans relative selection:bg-[#F4901E] selection:text-white pb-12">
      ...
      {/* Grade de Ambientes - Otimizada para Celular (1 coluna bem espaçada) e Desktop (4 colunas) */}
      <main className="flex-1 px-3 sm:px-6 max-w-[2400px] mx-auto w-full z-10">
        {filteredRooms.length === 0 ? (
          <div className="bg-white rounded-2xl border border-[#CBD5E1] p-8 text-center flex flex-col items-center justify-center my-4 shadow-xs">
            ...
            <button
              onClick={() => setFilterStatus('todos')}
              className="mt-3 px-4 py-2 bg-[#0F2A52] text-white rounded-xl text-xs font-bold uppercase hover:bg-[#1D4E8C]"
            >
              Ver Todas as Salas
            </button>
          </div>
              ...
              {filteredRooms.map((room, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`rounded-2xl p-3 sm:p-3.5 bg-white border transition-all flex flex-col justify-between shadow-xs hover:shadow-md relative overflow-hidden ${
                      hasObs
                        ? 'border-[#F4901E] ring-2 ring-[#F4901E]/30'
                        : isEmAula
                        ? 'border-red-300'
                        : isLivre
                        ? 'border-emerald-300'
                  ...
                  >
                    {/* Borda superior colorida indicando status */}
                    <div
                    ...
                    <div>
                      {/* Topo do Card: Nome da Sala + Status Badge Grande e Legível */}
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div className="min-w-0 flex-1">
                          <h2 className="text-[13px] sm:text-sm font-black uppercase tracking-tight text-[#0F2A52] leading-tight break-words">
                            {room.nomeFormatado}
                          </h2>
                        </div>

                        {/* Status Badge Claro e Chamativo para Celular */}
                        <div className="shrink-0">
                          {isEmAula && (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-red-100 text-red-800 border border-red-200 shadow-xs">
                              <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
                              Em Aula
                            </span>
                          )}
                          {isLivre && (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-emerald-100 text-emerald-800 border border-emerald-200 shadow-xs">
                              <span className="w-2 h-2 rounded-full bg-emerald-500" />
                              Livre
                            </span>
                          )}
                          ...
                          {room.status === 'sem_aula' && (
                            <span className="inline-flex items-center px-1.5 py-0.5 rounded-md text-[9px] font-bold text-slate-500 bg-slate-100">
                              Sem Aula
                            </span>
                          )}
                        </div>
                      ...
                      {/* Informativo Operacional Objetivo (Banner de Destaque no Celular) */}
                      <div
                        className={`rounded-xl px-2.5 py-2 mb-2.5 text-xs sm:text-[12px] font-bold flex items-center gap-2 border ${
                          isEmAula
                      ...
                      >
                        {isEmAula && (
                          <>
                            ...
                            <div className="min-w-0 flex-1 leading-snug">
                              Ocupada até as <strong>{room.currentClass?.fim}</strong>
                        ...
                        {isLivre && (
                          <>
                            ...
                            <div className="min-w-0 flex-1 leading-snug">
                              {room.nextClass ? (
                                <>
                                  Liberada até as <strong>{room.nextClass.inicio}</strong>
                                  <span className="block text-[10px] font-normal text-emerald-800 truncate">
                                    Próxima aula: {room.nextClass.aula.turma}
                                  </span>
                                </>
                              ) : (
                                <>
                        ...
                        {isConcluido && (
                          <>
                            <CheckCircle2 className="w-4 h-4 text-[#1D4E8C] shrink-0" />
                            <div className="min-w-0 flex-1 leading-snug">
                      ...
                      {/* Observação da Gestão (Alto Contraste no Smartphone) */}
                      {hasObs && (
                        <div className="mb-2.5 p-2.5 rounded-xl bg-amber-50 border-2 border-[#F4901E] flex items-start gap-2 text-xs font-bold text-amber-950 shadow-xs">
                      ...
                      {/* Horários das Aulas do Dia (Legível e Espaçado no Celular) */}
                      <div className="flex flex-col gap-1">
                        <div className="text-[9px] font-black uppercase tracking-wider text-[#6B7280] mb-0.5 flex items-center justify-between">
                          <span>Horários do Dia ({room.classes.length})</span>
                          <span className="font-semibold text-[#1D4E8C] lowercase">hoje</span>
                        </div>

                        {room.classes.length === 0 ? (
                          <div className="py-1 text-xs text-[#6B7280] italic">
                              ...
                              return (
                                <div
                                  ...
                                  className={`px-2.5 py-1.5 rounded-xl border transition-all ${
                                    isCurrent
                                      ? 'bg-red-50 border-red-300 ring-1 ring-red-300 font-bold'
                                      : isPast
                                      ? 'bg-slate-50/70 border-slate-200 opacity-60'
                                      : 'bg-[#F8FAFC] border-[#E2E8F0]'
                                  }`}
                                >
                                  {/* Linha 1: Horário + Turno + Badge Agora */}
                                  <div className="flex items-center justify-between text-xs mb-0.5">
                                    <div className="flex items-center gap-1 font-mono font-bold text-[#0F2A52]">
                                      <Clock className={`w-3 h-3 ${isCurrent ? 'text-red-500' : 'text-[#F4901E]'}`} />
                                      <span>{cls.inicio} — {cls.fim}</span>
                                      <span className="text-[8px] uppercase px-1.5 py-0.2 rounded bg-[#DBEAFE] text-[#1D4E8C] font-sans font-black">
                                        {cls.turno}
                                      </span>
                                    </div>
                                    {isCurrent && (
                                  ...
                                  {/* Linha 2: Turma + Instrutor */}
                                  <div className="flex items-center justify-between text-[11px] text-[#374151] gap-1">
                                    <span className="font-bold text-[#0F2A52] uppercase truncate">
                                      {cls.aula.turma}
                                    </span>
                                    {cls.aula.instrutor && (
                    ...
                    {/* Rodapé do Card */}
                    <div className="mt-2.5 pt-1.5 border-t border-[#E5E7EB] flex items-center justify-between text-[10px] text-[#6B7280]">
                      <span className="font-bold">
                        {room.classes.length > 0 ? `${room.classes.length} turno(s) no dia` : 'Sem agendamentos'}
                      </span>
                      {room.observacao?.atualizadoPor && (
                        <span className="text-[9px] truncate max-w-[130px]" title={room.observacao.atualizadoPor}>
